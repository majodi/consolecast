...
/**
 * @brief Open USB device with requested VID/PID, or first available device (VID and PID both 0)
 *
 * This function has two regular return paths:
 * 1. USB device with matching VID/PID (or first available when VID and PID both 0) is already opened by this driver: allocate new CDC device on top of the already opened USB device.
 * 2. USB device with matching VID/PID (or first available when VID and PID both 0) is NOT opened by this driver yet: poll USB connected devices until it is found.
 *
...
static esp_err_t cdc_acm_find_and_open_usb_device(uint16_t vid, uint16_t pid, int timeout_ms, cdc_dev_t **dev)
{
    ...
    *dev = calloc(1, sizeof(cdc_dev_t));
    ...
    SLIST_FOREACH(cdc_dev, &p_cdc_acm_obj->cdc_devices_list, list_entry) {
        ...
        if ((vid == 0 && pid == 0) || (device_desc->idVendor == vid && device_desc->idProduct == pid)) {
            ESP_LOGI(TAG, "Found opened CDC device: %d (VID) %d (PID)", device_desc->idVendor, device_desc->idProduct);
            // Return path 1:
    ...
    while (true) {
        ...
        for (int i = 0; i < num_of_devices; i++) {
            ...
            // if (device_desc->idVendor == vid && device_desc->idProduct == pid) {
            if ((vid == 0 && pid == 0) || (device_desc->idVendor == vid && device_desc->idProduct == pid)) {
                ESP_LOGI(TAG, "Found connected CDC device: %d (VID) %d (PID)", device_desc->idVendor, device_desc->idProduct);
                // Return path 2:
